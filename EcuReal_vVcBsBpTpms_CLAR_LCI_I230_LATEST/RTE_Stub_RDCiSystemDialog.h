...
private: System::Void timerRDCiCyclicTask_Tick(System::Object^  sender, System::EventArgs^  e)
{
  ...
  Rdci_UHRZEIT_DATUM_Type myUhrzeitDatum;
   ...
   if(++ucTime1Second >= (1000 / timerRDCiCyclicTask->Interval)){
      ...
      if(ucGetColOfWP(0) < 4){ labelMuteFlTimerValue->Text = System::String::Format("{0} (FL)", ushGetMuteTimerValueWUM(ucGetColOfWP(0))); }
      else{ labelMuteFlTimerValue->Text = System::String::Format("{0} (Slot 0)", ushGetMuteTimerValueWUM(0));              }

      if(ucGetColOfWP(1) < 4){ labelMuteFrTimerValue->Text = System::String::Format("{0} (FR)", ushGetMuteTimerValueWUM(ucGetColOfWP(1))); }
      else{ labelMuteFrTimerValue->Text = System::String::Format("{0} (Slot 1)", ushGetMuteTimerValueWUM(1));              }

      if(ucGetColOfWP(2) < 4){ labelMuteRlTimerValue->Text = System::String::Format("{0} (RL)", ushGetMuteTimerValueWUM(ucGetColOfWP(2))); }
      else{ labelMuteRlTimerValue->Text = System::String::Format("{0} (Slot 2)", ushGetMuteTimerValueWUM(2));              }

      if(ucGetColOfWP(3) < 4){ labelMuteRrTimerValue->Text = System::String::Format("{0} (RR)", ushGetMuteTimerValueWUM(ucGetColOfWP(3))); }
      else{ labelMuteRrTimerValue->Text = System::String::Format("{0} (Slot 3)", ushGetMuteTimerValueWUM(3));              }

      if(ucGetColOfWP(0) < 4){ labelLoBatCounterFL->Text = System::String::Format("{0}", ucGetLoBatCounterValueWUM(ucGetColOfWP(0))); }
      else{ labelLoBatCounterFL->Text = System::String::Format("no WP"); }

      if(ucGetColOfWP(1) < 4){ labelLoBatCounterFR->Text = System::String::Format("{0}", ucGetLoBatCounterValueWUM(ucGetColOfWP(1))); }
      else{ labelLoBatCounterFR->Text = System::String::Format("no WP"); }

      if(ucGetColOfWP(2) < 4){ labelLoBatCounterRL->Text = System::String::Format("{0}", ucGetLoBatCounterValueWUM(ucGetColOfWP(2))); }
      else{ labelLoBatCounterRL->Text = System::String::Format("no WP"); }

      if(ucGetColOfWP(3) < 4){ labelLoBatCounterRR->Text = System::String::Format("{0}", ucGetLoBatCounterValueWUM(ucGetColOfWP(3))); }
      else{ labelLoBatCounterRR->Text = System::String::Format("no WP"); }
   ...
   if(NvmDscCodierdatenAllgemein_ReadByte( C_FUNKTION_REIFENPANNENERKENNUNG_AKTIV_BYTE, C_FUNKTION_REIFENPANNENERKENNUNG_AKTIV_MASK ) == AKTIV_RDC ){
    checkRDCi_aktiv->Checked = TRUE;
   }
   else{
    checkRDCi_aktiv->Checked = FALSE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_T_PREWARN_NC_BYTE, C_RDCI_T_PREWARN_NC_MASK ) == 0 ){
    checkBoxCRdciTPrewarnNc->Checked = FALSE;
   }
   else{
    checkBoxCRdciTPrewarnNc->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_MAX_THRESHOLD_BYTE, C_RDCI_MAX_THRESHOLD_MASK ) == 0 ){
    checkBoxCRdciMaxThreshold->Checked = FALSE;
   }
   else{
    checkBoxCRdciMaxThreshold->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_PREWARN_ENABLE_BYTE, C_RDCI_PREWARN_ENABLE_MASK ) == 0 ){
    checkBoxCRdciPrewarnEnable->Checked = FALSE;
   }
   else{
    checkBoxCRdciPrewarnEnable->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_STAT_INIT_BYTE, C_RDCI_STAT_INIT_MASK ) == 0 ){
    checkBoxCRdciStatInit->Checked = FALSE;
   }
   else{
    checkBoxCRdciStatInit->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_NUM_PREWARN_DETECT_BYTE, C_RDCI_NUM_PREWARN_DETECT_MASK ) == 0 ){
    checkBoxCRdciNumPrewarnDetect->Checked = FALSE;
   }
   else{
    checkBoxCRdciNumPrewarnDetect->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_PREWARN_IGNITION_BYTE, C_RDCI_PREWARN_IGNITION_MASK ) == 0 ){
    checkBoxCRdciPrewarnIgnition->Checked = FALSE;
   }
   else{
    checkBoxCRdciPrewarnIgnition->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_PANNE_BEF_POS_BYTE, C_RDCI_PANNE_BEF_POS_MASK ) == 0 ){
    checkBoxCRdciPanneBefPos->Checked = FALSE;
   }
   else{
    checkBoxCRdciPanneBefPos->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_FAST_DEFLATE_ENABLE_BYTE, C_RDCI_FAST_DEFLATE_ENABLE_MASK ) == 0 ){
    checkBoxCRdciFastDeflateEnable->Checked = FALSE;
   }
   else{
    checkBoxCRdciFastDeflateEnable->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_TYR_ID_FILT_GW_BYTE, C_RDCI_TYR_ID_FILT_GW_MASK ) == 0 ){
    checkBoxCRdciTyrIdFiltGw->Checked = FALSE;
   }
   else{
    checkBoxCRdciTyrIdFiltGw->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_RESET_PLAUSI_BYTE, C_RDCI_RESET_PLAUSI_MASK ) == 0 ){
    checkBoxCRdciResetPlausi->Checked = FALSE;
   }
   else{
    checkBoxCRdciResetPlausi->Checked = TRUE;
   }

   ucMarket = NvmDscCodierdatenRDCi_ReadByte( C_RDCI_TPMS_MARKET_BYTE, C_RDCI_TPMS_MARKET_MASK ) >> 2;

   if(listBoxCRdciTpmsMarket->SelectedIndex > UNDEFINED ){
    if (ucMarket == UNDEFINED ){
      listBoxCRdciTpmsMarket->SetSelected( 4, TRUE );
      }
      else{
      listBoxCRdciTpmsMarket->SetSelected(ucMarket, TRUE );
      }
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_SENSOR_FOREIGN_AK_RDK_BYTE, C_RDCI_SENSOR_FOREIGN_AK_RDK_MASK ) == 0 ){
    checkBoxCRdciSensorForeignAkRdk->Checked = FALSE;
   }
   else{
    checkBoxCRdciSensorForeignAkRdk->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_SENSOR_LOC_SYNC_BYTE, C_RDCI_SENSOR_LOC_SYNC_MASK ) == 0 ){
    checkBoxCRdciSensorLocSync->Checked = FALSE;
   }
   else{
    checkBoxCRdciSensorLocSync->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_ERFS_ENABLE_BYTE, C_RDCI_ERFS_ENABLE_MASK ) == 0 ){
    checkBoxCRdciErfsEnable->Checked = FALSE;
   }
   else{
    checkBoxCRdciErfsEnable->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_TREF_SEASONAL_ADJUSTMENT_ENABLE_BYTE, C_RDCI_TREF_SEASONAL_ADJUSTMENT_ENABLE_MASK ) == 0 ){
    checkBoxCRdciTrefSeasonalAdjustment->Checked = FALSE;
   }
   else{
    checkBoxCRdciTrefSeasonalAdjustment->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_DISP_RESET_BYTE, C_RDCI_DISP_RESET_MASK ) == 0 ){
    checkBoxCRdciDispReset->Checked = FALSE;
   }
   else{
    checkBoxCRdciDispReset->Checked = TRUE;
   }

  numericUpDownCRdciCorHoldOffTime->Value       = ((NvmDscCodierdatenRDCi_ReadByte( C_RDCI_COR_HOLD_OFF_TIME_BYTE, C_RDCI_COR_HOLD_OFF_TIME_MASK ) >> 5) * 5);
  numericUpDownCRdciTRefShift->Value            = NvmDscCodierdatenRDCi_ReadByte( C_RDCI_T_REF_SHIFT_BYTE, C_RDCI_T_REF_SHIFT_MASK );
  numericUpDownCRdciUiaThC->Value               = NvmDscCodierdatenRDCi_ReadByte( C_RDCI_UIA_TH_C_BYTE, C_RDCI_UIA_TH_C_MASK );
  numericUpDownCRdciUiwThC->Value               = NvmDscCodierdatenRDCi_ReadByte( C_RDCI_UIW_TH_C_BYTE, C_RDCI_UIW_TH_C_MASK );
  numericUpDownCRdciUiaThNc->Value              = NvmDscCodierdatenRDCi_ReadByte( C_RDCI_UIA_TH_NC_BYTE, C_RDCI_UIA_TH_NC_MASK );
  numericUpDownCRdciUiwThNc->Value              = NvmDscCodierdatenRDCi_ReadByte( C_RDCI_UIW_TH_NC_BYTE, C_RDCI_UIW_TH_NC_MASK );
  numericUpDownCRdciPanneTh->Value              = (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_PANNE_TH_BYTE, C_RDCI_PANNE_TH_MASK ) * 25);
  numericUpDownCRdciMinRcpFa->Value             = (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_MIN_RCP_FA_BYTE, C_RDCI_MIN_RCP_FA_MASK ) * 25);
  numericUpDownCRdciMinRcpRa->Value             = (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_MIN_RCP_RA_BYTE, C_RDCI_MIN_RCP_RA_MASK ) * 25);
  numericUpDownCRdciDeltaPLR->Value             = (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_DELTA_P_L_R_BYTE, C_RDCI_DELTA_P_L_R_MASK ) * 25);
  numericUpDownCRdciPInitRangeMax->Value        = (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_P_INIT_RANGE_MAX_BYTE, C_RDCI_P_INIT_RANGE_MAX_MASK ) * 25);
  numericUpDownCRdciTimeoutPrewarn->Value       = NvmDscCodierdatenRDCi_ReadByte( C_RDCI_TIMEOUT_PREWARN_BYTE, C_RDCI_TIMEOUT_PREWARN_MASK );
  numericUpDownCRdciDtAmbPrewarn->Value         = NvmDscCodierdatenRDCi_ReadByte( C_RDCI_DT_AMB_PREWARN_BYTE, C_RDCI_DT_AMB_PREWARN_MASK );
  numericUpDownCRdciDpToIPmin->Value            = (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_DP_TO_I_PMIN_BYTE, C_RDCI_DP_TO_I_PMIN_MASK ) * 25);
  numericUpDownCRdciLearnLocateConfig->Value    = ((NvmDscCodierdatenRDCi_ReadByte( C_RDCI_LEARN_LOCATE_CONFIG_BYTE_0,  C_RDCI_LEARN_LOCATE_CONFIG_MASK_0  ) <<  0) + (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_LEARN_LOCATE_CONFIG_BYTE_1,  C_RDCI_LEARN_LOCATE_CONFIG_MASK_1  ) <<  8));
  numericUpDownCRdciTrefSeasAdjConfig->Value    = ((NvmDscCodierdatenRDCi_ReadByte( C_RDCI_TREF_SEAS_ADJ_CONFIG_BYTE_0, C_RDCI_TREF_SEAS_ADJ_CONFIG_MASK_0 ) <<  0) + (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_TREF_SEAS_ADJ_CONFIG_BYTE_1, C_RDCI_TREF_SEAS_ADJ_CONFIG_MASK_1 ) <<  8) + (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_TREF_SEAS_ADJ_CONFIG_BYTE_2, C_RDCI_TREF_SEAS_ADJ_CONFIG_MASK_2 ) <<  16) + (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_TREF_SEAS_ADJ_CONFIG_BYTE_3, C_RDCI_TREF_SEAS_ADJ_CONFIG_MASK_3 ) << 24));
  numericUpDownCRdciXminCoolTire->Value         = NvmDscCodierdatenRDCi_ReadByte( C_RDCI_XMIN_COOL_TIRE_BYTE, C_RDCI_XMIN_COOL_TIRE_MASK );
  numericUpDownCRdciMaxCorTime->Value           = NvmDscCodierdatenRDCi_ReadByte( C_RDCI_MAX_COR_TIME_BYTE, C_RDCI_MAX_COR_TIME_MASK );
  numericUpDownCRdciMaxCorRcp->Value            = (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_MAX_COR_RCP_BYTE, C_RDCI_MAX_COR_RCP_MASK ) * 25);
  numericUpDownCRdciThCTol->Value               = (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_TH_C_TOL_BYTE, C_RDCI_TH_C_TOL_MASK ) * 10);
  numericUpDownCRdciThNCTol->Value              = ((NvmDscCodierdatenRDCi_ReadByte( C_RDCI_TH_NC_TOL_BYTE, C_RDCI_TH_NC_TOL_MASK ) >> 4) * 10);
  numericUpDownCRdciLongHoldTime->Value         = (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_LONG_HOLD_TIME_BYTE, C_RDCI_LONG_HOLD_TIME_MASK) + 5);
  numericUpDownCRdciShortHoldTime->Value        = ((NvmDscCodierdatenRDCi_ReadByte( C_RDCI_SHORT_HOLD_TIME_BYTE, C_RDCI_SHORT_HOLD_TIME_MASK ) >> 4) + 1) ;
  numericUpDownCRdciDispConfTimeout->Value      = (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_DISP_CONF_TIMEOUT_BYTE, C_RDCI_DISP_CONF_TIMEOUT_MASK ) >> 4) ;
  
   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_ECO_RCP_ENABLE_BYTE, C_RDCI_ECO_RCP_ENABLE_MASK ) == 0 ){
    checkBoxCRdciEcoRcpEnable->Checked = FALSE;
   }
   else{
    checkBoxCRdciEcoRcpEnable->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_RID_ENABLE_BYTE, C_RDCI_RID_ENABLE_MASK ) == 0 ){
    checkBoxCRdciRidEnable->Checked = FALSE;
   }
   else{
    checkBoxCRdciRidEnable->Checked = TRUE;
   }

   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_DEFAULT_LOAD_COND_BYTE, C_RDCI_DEFAULT_LOAD_COND_MASK ) == 0 ){
    checkBoxCRdciDefaultLoadCond->Checked = FALSE;
   }
   else{
    checkBoxCRdciDefaultLoadCond->Checked = TRUE;
   }

   ucPlacard = (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_ERFS_PLACARD_SOURCE_BYTE, C_RDCI_ERFS_PLACARD_SOURCE_MASK ) >> 1);

   if(listBoxCRdciErfsPlacardSource->SelectedIndex > REMOTE_UPDATE ){
    listBoxCRdciErfsPlacardSource->SetSelected(ucPlacard, TRUE );
   }
   ...
   if(NvmDscCodierdatenRDCi_ReadByte( C_RDCI_SPEED_CCM_ENABLE_BYTE, C_RDCI_SPEED_CCM_ENABLE_MASK ) == 0 ){
    checkBoxCRdciSpeedCcmEnable->Checked = FALSE;
   }
   else{
    checkBoxCRdciSpeedCcmEnable->Checked = TRUE;
   }
  
  numericUpDownCRdciSpeedCcmTh->Value                           = (ImpTypeValCRdciSpeedCcmTh) ((NvmDscCodierdatenRDCi_ReadByte( C_RDCI_SPEED_CCM_TH_BYTE_0, C_RDCI_SPEED_CCM_TH_MASK_0 ) << 0) + (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_SPEED_CCM_TH_BYTE_1, C_RDCI_SPEED_CCM_TH_MASK_1 ) << 8) );
  numericUpDownCRdciSpeedCcmTime->Value                         = (uint8) NvmDscCodierdatenRDCi_ReadByte( C_RDCI_SPEED_CCM_TIME_BYTE, C_RDCI_SPEED_CCM_TIME_MASK );
  numericUpDownCRdciSpeedCcmHyst->Value                         = (uint8) NvmDscCodierdatenRDCi_ReadByte( C_RDCI_SPEED_CCM_HYST_BYTE, C_RDCI_SPEED_CCM_HYST_MASK );
  ...
  numericUpDownCRdciSpeedCcmPressOffsetRa->Value                = (ImpTypeValCRdciSpeedCcmPressOffsetRa) ((NvmDscCodierdatenRDCi_ReadByte( C_RDCI_SPEED_CCM_PRESS_OFFSET_RA_BYTE_0, C_RDCI_SPEED_CCM_PRESS_OFFSET_RA_MASK_0 ) << 0) + (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_SPEED_CCM_PRESS_OFFSET_RA_BYTE_1, C_RDCI_SPEED_CCM_PRESS_OFFSET_RA_MASK_1 ) << 8) );

  numericUpDownCRdciParkSupExtParkSupConfig->Value              = (uint8)  NvmDscCodierdatenRDCi_ReadByte( C_RDCI_PARK_SUP_EXT_PARK_SUP_CONFIG_BYTE, C_RDCI_PARK_SUP_EXT_PARK_SUP_CONFIG_MASK );
  numericUpDownCRdciParkSupMobilityLossThresholdValue->Value    = (uint16) (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_PARK_SUP_MOBILITY_LOSS_THRESHOLD_BYTE, C_RDCI_PARK_SUP_MOBILITY_LOSS_THRESHOLD_MASK ) * 100);
  numericUpDownCRdciParkSupNotificationThresholdValueC->Value   = (uint8)  NvmDscCodierdatenRDCi_ReadByte( C_RDCI_PARK_SUP_NOTIFICATION_THRESHOLD_VALUE_C_BYTE, C_RDCI_PARK_SUP_NOTIFICATION_THRESHOLD_VALUE_C_MASK );
  numericUpDownCRdciParkSupNotificationThresholdValueNc->Value  = (uint8)  NvmDscCodierdatenRDCi_ReadByte( C_RDCI_PARK_SUP_NOTIFICATION_THRESHOLD_VALUE_NC_BYTE, C_RDCI_PARK_SUP_NOTIFICATION_THRESHOLD_VALUE_NC_MASK );
  numericUpDownCRdciParkSupTolNoTempComp->Value                 = (uint8)  (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_PARK_SUP_TOL_NO_TEMP_COMP_BYTE, C_RDCI_PARK_SUP_TOL_NO_TEMP_COMP_MASK ) * 10);
  numericUpDownCRdciParkSupTolTempComp->Value                   = (uint8)  (NvmDscCodierdatenRDCi_ReadByte( C_RDCI_PARK_SUP_TOL_TEMP_COMP_BYTE, C_RDCI_PARK_SUP_TOL_TEMP_COMP_MASK ) * 10);
  numericUpDownCRdciParkSupWarningThresholdValueC->Value        = (uint8)  NvmDscCodierdatenRDCi_ReadByte( C_RDCI_PARK_SUP_WARNING_THRESHOLD_VALUE_C_BYTE, C_RDCI_PARK_SUP_WARNING_THRESHOLD_VALUE_C_MASK );
  numericUpDownCRdciParkSupWarningThresholdValueNc->Value       = (uint8)  NvmDscCodierdatenRDCi_ReadByte( C_RDCI_PARK_SUP_WARNING_THRESHOLD_VALUE_NC_BYTE, C_RDCI_PARK_SUP_WARNING_THRESHOLD_VALUE_NC_MASK );
  ...
  HistPos3Val -> Text = System::String::Format("{0}", ucWheelPos);
   ...
   textBoxWatoMinutes->Text =  System::String::Format("{0}", ushGetWatoTimeoutValueWAM());
   textBoxAllocFailCnt->Text = System::String::Format("{0}", ucGetAutoLocFailedCounterWUM());
   textBoxParkingTime->Text = System::String::Format("{0}", ulGetStopTimeDM());
  myUhrzeitDatum = GETtTimeDateEE( Rte_Inst_CtApHufTpmsSWC );
   textBoxUhrzeitDatumIntern->Text = System::String::Format("{0:00}.{1:00}.{2:00} {3:00}:{4:00}:{5:00}", myUhrzeitDatum.DISP_DATE_DAY, myUhrzeitDatum.DISP_DATE_MON, myUhrzeitDatum.DISP_DATE_YR, myUhrzeitDatum.DISP_HR, myUhrzeitDatum.DISP_MN, myUhrzeitDatum.DISP_SEC);
  pushAbsCounters = GetPointerToAbsCountersWAM();
  labelAbsCounterFL->Text = System::String::Format("{0}", pushAbsCounters[0]);
  labelAbsCounterFR->Text = System::String::Format("{0}", pushAbsCounters[1]);
  labelAbsCounterRL->Text = System::String::Format("{0}", pushAbsCounters[2]);
  labelAbsCounterRR->Text = System::String::Format("{0}", pushAbsCounters[3]);
	uint8 ucTyreListRow;
	float ucFA_PartLoad = CINIT_DISP_INVALID, ucRA_PartLoad= CINIT_DISP_INVALID, ucFA_FullLoad= CINIT_DISP_INVALID, ucRA_FullLoad= CINIT_DISP_INVALID, ucFA_EcoLoad= CINIT_DISP_INVALID, ucRA_EcoLoad= CINIT_DISP_INVALID;
	uint8 ucReifentyp;
   uint8 ucSpeedIndex;
   uint8 ucSaisonVal;
    
	MaxTyreTableVal -> Text = System::String::Format("{0}", GetMaxTyreTablePos());
	
	ucTyreListRow = System::Decimal::ToByte(SelectTyreListElement->Value);
	
	if (ucTyreListRow != CTLIST_INDEX_INVALID){
		ucFA_PartLoad = (((float)(GetFrontAxleSetPressTyreList(REQ_PART_LOAD, ucTyreListRow)))/40);
		ucRA_PartLoad = (((float)(GetRearAxleSetPressTyreList(REQ_PART_LOAD, ucTyreListRow)))/40);
		ucFA_FullLoad = (((float)(GetFrontAxleSetPressTyreList(REQ_FULL_LOAD, ucTyreListRow)))/40);
		ucRA_FullLoad = (((float)(GetRearAxleSetPressTyreList(REQ_FULL_LOAD, ucTyreListRow)))/40);
		ucFA_EcoLoad = (((float)(GetFrontAxleSetPressTyreList(REQ_ECO_LOAD, ucTyreListRow)))/40);
		ucRA_EcoLoad = (((float)(GetRearAxleSetPressTyreList(REQ_ECO_LOAD, ucTyreListRow)))/40);
	  
		FrontAxlePartLoadVal -> Text = System::String::Format("{0}", ucFA_PartLoad);
		RearAxlePartLoadVal -> Text = System::String::Format("{0}", ucRA_PartLoad);
		FrontAxleFullLoadVal -> Text = System::String::Format("{0}", ucFA_FullLoad);
		RearAxleFullLoadVal -> Text = System::String::Format("{0}", ucRA_FullLoad);
		FrontAxleEcoLoadVal -> Text = System::String::Format("{0}", ucFA_EcoLoad);
		RearAxleEcoLoadVal -> Text = System::String::Format("{0}", ucRA_EcoLoad);
		ReifenbreiteVal -> Text = System::String::Format("{0}", (GetReifenbreiteRaw(ucTyreListRow) *10 +95));
		KarkasseVal -> Text = System::String::Format("{0}", GetKarkasseRaw(ucTyreListRow) == 0 ? "Normal_Load" : "XL");
		WerkVal  -> Text = System::String::Format("{0}", GetWerksauslieferungRaw(ucTyreListRow) == 0 ? "Nein" : "Ja");
		QuerVal  -> Text = System::String::Format("{0}", GetQuerschnittRaw(ucTyreListRow) * 5 + 10);
      ucReifentyp = GetReifentypRaw(ucTyreListRow);
		DurchmVal  -> Text = System::String::Format("{0}", GetDurchmesserRaw(ucTyreListRow) + 12);
      ucSpeedIndex = GetGeschwIndexRaw(ucTyreListRow);
		TragVal  -> Text = System::String::Format("{0}", GetTragfaehigkeitRaw(ucTyreListRow) + 75);
      ucSaisonVal = GetSaisonRaw(ucTyreListRow);
      switch(ucReifentyp){
   ...
   ushTemp1 = GETTyreDimDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 0 );
   ushTemp2 = GETTyreDimDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 1 );
   ushTemp1 = (((ushTemp1 >> 4) - 6) * 100) + (((ushTemp1 & 0x0F) - 6) * 10) + ((ushTemp2 >> 4) - 6);  
  
   textRID_Reifenbreite1->Text = System::String::Format("{0}", ushTemp1);

   ushTemp1 = GETTyreDimDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 1 );
   ushTemp2 = GETTyreDimDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 2 );
   ushTemp1 = (((ushTemp1 & 0x0F) - 6) * 10) + ((ushTemp2 >> 4) - 6);

   textRID_Querschnitt1->Text = System::String::Format("{0}", ushTemp1);

   ushTemp1 = GETTyreDimDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 2 );
   ushTemp2 = GETTyreDimDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 3 );
   ushTemp1 = (((ushTemp1 & 0x0F) - 6) * 10) + ((ushTemp2 >> 4) - 6);

   textRID_Durchmesser1->Text = System::String::Format("{0}", ushTemp1);

   ushTemp1 = GETTyreDimDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 3 );
   ushTemp2 = GETTyreDimDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 4 );
   ushTemp1 = ((uint16)(ushTemp1 & 0x0F) << 3) | (uint16)((ushTemp2 & 0xE0) >> 5);

   textRID_Tragfähigkeit1->Text = System::String::Format("{0}", ushTemp1);

   ushTemp1 = GETTyreDimDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 4 );
   ushTemp1 = (ushTemp1 & 0x1F) + 0x40;

   textRID_GeschwIndex1->Text = System::String::Format("{0}", ushTemp1);

   ushTemp1 = GETTyreDimDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 5 );
   ushTemp1 = (uint16)((ushTemp1 & 0xC0) >> 6);

   textRID_Reifentyp1->Text = System::String::Format("{0}", ushTemp1);

   ushTemp1 = GETTyreDimDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 5 );
   ushTemp1 = (uint16)((ushTemp1 & 0x0C) >> 2);

   textRID_Saison1->Text = System::String::Format("{0}", ushTemp1);

   ushTemp1 = GETTyreDimDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 5 );
   ushTemp1 = (uint16)((ushTemp1 & 0x03));
  
   textRID_Karkasse1->Text = System::String::Format("{0}", ushTemp1);
  
   ushTemp1 = GETTyreDOTDataEE( Rte_Inst_CtApHufTpmsSWC, 0, 0 );
   textRID_DOT1 -> Text = System::String::Format("{0}", ushTemp1);
   ...
   if(bGetBandmodeBM() == TRUE ){
    checkBoxBandmodeAktiv->Checked = true;
   }
   else{
    checkBoxBandmodeAktiv->Checked = false;
   }

  // ************ Bandmode Service anzeigen *************
   if(bGetStateBitBM( cushTestEigenradFahrtBM ) == TRUE ){
    checkBoxBandmodeServiceTestErFahrt->Checked = true;
   }
   else{
    checkBoxBandmodeServiceTestErFahrt->Checked = false;
   }

   if(bGetStateBitBM( cushTestEigenradFahrtNoSpeedBM ) == TRUE ){
    checkBoxBandmodeServiceTestErFahrtNoSpeed->Checked = true;
   }
   else{
    checkBoxBandmodeServiceTestErFahrtNoSpeed->Checked = false;
   }
   if(bGetStateBitBM ( cushErFahrtVThresBM ) == TRUE ){
    checkBoxBandmodeStatusErFahrtVThres->Checked = true;
   }
   else{
    checkBoxBandmodeStatusErFahrtVThres->Checked = false;
   }

   if(bGetStateBitBM ( cushReceptionReFlBM ) == TRUE ){
    checkBoxBandmodeStatusRecReFL->Checked = false;
   }
   else{
    checkBoxBandmodeStatusRecReFL->Checked = true;
   }

   if(bGetStateBitBM ( cushReceptionReFrBM ) == TRUE ){
    checkBoxBandmodeStatusRecReFR->Checked = false;
   }
   else{
    checkBoxBandmodeStatusRecReFR->Checked = true;
   }

   if(bGetStateBitBM ( cushReceptionReRlBM ) == TRUE ){
    checkBoxBandmodeStatusRecReRL->Checked = false;
   }
   else{
    checkBoxBandmodeStatusRecReRL->Checked = true;
   }

   if(bGetStateBitBM ( cushReceptionReRrBM ) == TRUE ){
    checkBoxBandmodeStatusRecReRR->Checked = false;
   }
   else{
    checkBoxBandmodeStatusRecReRR->Checked = true;
   }
   if(bGetBitFahrzeugzustandFZZ( cKL_15_EIN ) == TRUE ){
    checkBoxKL_15_EIN->Checked = true;
   }
   else{
    checkBoxKL_15_EIN->Checked = false;
   }

   if(bGetBitFahrzeugzustandFZZ( cFAHRZEUG_FAEHRT ) == TRUE ){
    checkBoxFAHRZEUG_FAEHRT->Checked = true;
   }
   else{
    checkBoxFAHRZEUG_FAEHRT->Checked = false;
   }

   if(bGetBitFahrzeugzustandFZZ( cBM_VTHRES ) == TRUE ){
    checkBoxBM_VTHRES->Checked = true;
   }
   else{
    checkBoxBM_VTHRES->Checked = false;
   }

   switch(ucGetStatusConditionVehicleFZZ() ){
      case ST_CON_VEH_reserviert_0:
      case ST_CON_VEH_reserviert_13:
      case ST_CON_VEH_reserviert_14:
      labelStatusZustandFahrzeugWert->Text = "reserviert";
      break;
  
      case ST_CON_VEH_Parken_BN_niO:
      labelStatusZustandFahrzeugWert->Text = "Parken_BN_niO";
      break;

      case ST_CON_VEH_Parken_BN_iO:
      labelStatusZustandFahrzeugWert->Text = "Parken_BN_iO";
      break;

      case ST_CON_VEH_Standfunktionen_Kunde_nicht_im_FZG:
      labelStatusZustandFahrzeugWert->Text = "Standfunktionen_Kunde_nicht_im_FZG";
      break;

      case ST_CON_VEH_Wohnen:
      labelStatusZustandFahrzeugWert->Text = "Wohnen";
      break;

      case ST_CON_VEH_Pruefen_Analyse_Diagnose:
      labelStatusZustandFahrzeugWert->Text = "Pruefen_Analyse_Diagnose";
      break;

      case ST_CON_VEH_Fahrbereitschaft_herstellen:
      labelStatusZustandFahrzeugWert->Text = "Fahrbereitschaft_herstellen";
      break;

      case ST_CON_VEH_Fahren:
      labelStatusZustandFahrzeugWert->Text = "Fahren";
      break;

      case ST_CON_VEH_Fahrbereitschaft_beenden:
      labelStatusZustandFahrzeugWert->Text = "Fahrbereitschaft_beenden";
      break;

      case ST_CON_VEH_Signal_unbefuellt:
      default:
      labelStatusZustandFahrzeugWert->Text = "ungültig";
      break;
   }
   if(bGetBitBetriebszustandBZ( cEIGENRAD ) == TRUE ){
    checkBoxEigenrad->Checked = true;
    
    if ( bGetBitBetriebszustandBZ( cZUGEORDNET ) == TRUE ){
      checkBoxZugeordnet->Checked = true;
      }
      else{
      checkBoxZugeordnet->Checked = false;
      }
   }
   else{
    ClearBitBetriebszustandBZ( cZUGEORDNET );

    checkBoxEigenrad->Checked = false;
    checkBoxZugeordnet->Checked = false;
   }
 
  checkBoxERFinish->Checked = bGetBitBetriebszustandBZ( cER_FINISH );
  checkBoxTooMuchRE->Checked = bGetBitBetriebszustandBZ( cTOO_MUCH_RE );
  checkBoxDtcInactive->Checked = bGetBitBetriebszustandBZ( cDTC_INACTIVE );
  checkBoxLocNotPossible->Checked = bGetBitBetriebszustandBZ( cLOC_NOT_POSSIBLE );
  checkBoxLocInterrupted->Checked = bGetBitBetriebszustandBZ( cLOC_INTERRUPTED );
  checkBoxHistPreload->Checked = bGetBitBetriebszustandBZ( cHIST_PRELOAD );
  checkBoxZOFinish->Checked = bGetBitBetriebszustandBZ( cZO_FINISH );
  checkBoxTeileigenrad->Checked = bGetBitBetriebszustandBZ( cTEILEIGENRAD );
  checkBoxTimeout->Checked = bGetBitBetriebszustandBZ( cZO_TIMEOUT );
  checkBoxZwangsZo->Checked = bGetBitBetriebszustandBZ( cZWANGSZUORDNUNG );
  pucNewWheelPosOfId = pucGetNewWheelPosOfIdWAL_debug( Rte_Inst_CtApHufTpmsSWC, &ucPermState );
  labelPermutationStatusWert->Text = System::String::Format( "0x{0:X2}", ucPermState );
  
   if(bGetBitBetriebszustandBZ(cZO_FINISH) == TRUE ){
    checkBoxPermutationFLcolFLrow->Enabled = true;
    checkBoxPermutationFRcolFLrow->Enabled = true;
    checkBoxPermutationRLcolFLrow->Enabled = true;
    checkBoxPermutationRRcolFLrow->Enabled = true;
    checkBoxPermutationFLcolFRrow->Enabled = true;
    checkBoxPermutationFRcolFRrow->Enabled = true;
    checkBoxPermutationRLcolFRrow->Enabled = true;
    checkBoxPermutationRRcolFRrow->Enabled = true;
    checkBoxPermutationFLcolRLrow->Enabled = true;
    checkBoxPermutationFRcolRLrow->Enabled = true;
    checkBoxPermutationRLcolRLrow->Enabled = true;
    checkBoxPermutationRRcolRLrow->Enabled = true;
    checkBoxPermutationFLcolRRrow->Enabled = true;
    checkBoxPermutationFRcolRRrow->Enabled = true;
    checkBoxPermutationRLcolRRrow->Enabled = true;
    checkBoxPermutationRRcolRRrow->Enabled = true;
    checkBoxPermutationFLcolNewIDrow->Enabled = true;
    checkBoxPermutationFRcolNewIDrow->Enabled = true;
    checkBoxPermutationRLcolNewIDrow->Enabled = true;
    checkBoxPermutationRRcolNewIDrow->Enabled = true;
      ...
      switch(ucLoop){
        case cRadPosVL:
          switch(pucNewWheelPosOfId[ucLoop]){
            case cRadPosVL:
              checkBoxPermutationFLcolFLrow->Checked = true;
            break;

            case cRadPosVR:
              checkBoxPermutationFLcolFRrow->Checked = true;
              break;

            case cRadPosHL:
              checkBoxPermutationFLcolRLrow->Checked = true;
              break;

            case cRadPosHR:
              checkBoxPermutationFLcolRRrow->Checked = true;
              break;
            default:
  ...
  checkBoxRdciWarnruecknahme->Checked = bActualDtcState( tRDCiDtclist[cDemErrorRdciWarnruecknahme].ucStatus);
   ...
   if(ushTemp1 == cWUM_ENABLED){ labelWumActivityStatus->Text = "WUM Enabled"; }
   else if(ushTemp1 == cWUM_DISABLED){ labelWumActivityStatus->Text = "WUM Disabled"; }
   else if(ushTemp1 == cWUM_BM_DIENST_START){ labelWumActivityStatus->Text = "WUM Limited"; }
   else{}
  ...
  labelCorrTinitHigherEventValueSRA->Text = System::String::Format( "{0}", bCorrTinitEvent);
   ...
   if(*(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_0->value == comboBoxSelectMultiplexer->SelectedIndex){
    aucDbgMsg[0]  = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_0->value;
    aucDbgMsg[1]  = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_1->value;
    aucDbgMsg[2]  = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_2->value;
    aucDbgMsg[3]  = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_3->value;
    aucDbgMsg[4]  = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_4->value;
    aucDbgMsg[5]  = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_5->value;
    aucDbgMsg[6]  = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_6->value;
    aucDbgMsg[7]  = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_7->value;
    aucDbgMsg[8]  = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_8->value;
    aucDbgMsg[9]  = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_9->value;
    aucDbgMsg[10] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_10->value;
    aucDbgMsg[11] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_11->value;
    aucDbgMsg[12] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_12->value;
    aucDbgMsg[13] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_13->value;
    aucDbgMsg[14] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_14->value;
    aucDbgMsg[15] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_1_RDCI_DATA_15->value;
    aucDbgMsg[16] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_16->value;
    aucDbgMsg[17] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_17->value;
    aucDbgMsg[18] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_18->value;
    aucDbgMsg[19] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_19->value;
    aucDbgMsg[20] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_20->value;
    aucDbgMsg[21] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_21->value;
    aucDbgMsg[22] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_22->value;
    aucDbgMsg[23] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_23->value;
    aucDbgMsg[24] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_24->value;
    aucDbgMsg[25] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_25->value;
    aucDbgMsg[26] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_26->value;
    aucDbgMsg[27] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_27->value;
    aucDbgMsg[28] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_28->value;
    aucDbgMsg[29] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_29->value;
    aucDbgMsg[30] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_30->value;
    aucDbgMsg[31] = *(uint8*)&Rte_Inst_CtApHufTpmsSWC->RCyclicRDCiTask_PpFrPdu_FR_DBG_DSC_SUPP_2_RDCI_DATA_31->value;
