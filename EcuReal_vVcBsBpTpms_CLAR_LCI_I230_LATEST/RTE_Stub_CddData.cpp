...
static uint16 u16AbsQueueWritePos = 0xFFFF;
static uint16 u16RdcQueueWritePos = 0xFFFF;
static uint16 u16AbsQueueReadPos  = 0xFFFF;
static uint16 u16RdcQueueReadPos  = 0xFFFF;
...
uint16 PutCddAbsData(tCddAbsData* data){
   ...
   if(0xffff == u16AbsQueueWritePos){
      ...
      if(((u16AbsQueueWritePos + 1) % ABS_DATA_QUEUE_LENGTH) == u16AbsQueueReadPos){
         ...
         bLockPosition = false;
         if(u16AbsQueueWritePos > u16AbsQueueReadPos){queueFillStatus = u16AbsQueueWritePos   -  u16AbsQueueReadPos;}
         else                                        {queueFillStatus = ABS_DATA_QUEUE_LENGTH - (u16AbsQueueReadPos - u16AbsQueueWritePos);}
      }
   }
...
uint16 PutCddRdcData(tCddRdcData* data){
   ...
   if(0xFFFF == u16RdcQueueWritePos){
      ...
      if(((u16RdcQueueWritePos + 1) % RDC_DATA_QUEUE_LENGTH) == u16RdcQueueReadPos){
         ...
         bLockPosition = false;
         if(0xFFFF == u16RdcQueueReadPos){
             queueFillStatus = u16RdcQueueWritePos;
         }
         else{
            if(u16RdcQueueWritePos > u16RdcQueueReadPos){queueFillStatus = u16RdcQueueWritePos   -  u16RdcQueueReadPos;}
           else                                         {queueFillStatus = RDC_DATA_QUEUE_LENGTH - (u16RdcQueueReadPos - u16RdcQueueWritePos);}
         }
      }
   }
   if(false == bLockPosition){
...
FUNC(Std_ReturnType, RTE_CODE) RTE_Stub_Receive_CddAbsData(P2VAR(ImpTypeRecCddAbsData, AUTOMATIC, RTE_CTAPHUFTPMSSWC_APPL_VAR) data){
...
FUNC(Std_ReturnType, RTE_CODE) RTE_Stub_Receive_CddRdcData(P2VAR(ImpTypeRecCddRdcData, AUTOMATIC, RTE_CTAPHUFTPMSSWC_APPL_VAR) data){
   uint8 ucReturnValue  = 0xFF;
   bool  bReadQueueData = true;
   if(0xFFFF != u16RdcQueueWritePos){
...
void InitRecCddRdcData(void){
   ...
   for(ucLoop = 0; ucLoop < cMaxReElements; ucLoop++){
      ...
      case cRecEleIx_Slot3:
         ...
         if(cInvalidTelType == ucGetRePckgIdDM(ucLoop)){tRdcData[ucLoop].tRecCddRdcData.PCKG_ID = cTelTypeSELPAL;}
         else                                          {tRdcData[ucLoop].tRecCddRdcData.PCKG_ID = ucGetRePckgIdDM(ucLoop);}

         if(                   GETucLastWuPressureEE(Rte_Inst_CtApHufTpmsSWC, ucLoop) == cInvalidREpressure)                                                    {tRdcData[ucLoop].tRecCddRdcData.RDC_DT_1 = ((80 + GETucPAmbValEE(Rte_Inst_CtApHufTpmsSWC)) - 38);}
         else{
                 if( ((uint16) GETucLastWuPressureEE(Rte_Inst_CtApHufTpmsSWC, ucLoop) + (uint16) GETucPAmbValEE(Rte_Inst_CtApHufTpmsSWC)) < 40)                 {tRdcData[ucLoop].tRecCddRdcData.RDC_DT_1 = cReAkPresUnderflow;}
            else if((((uint16) GETucLastWuPressureEE(Rte_Inst_CtApHufTpmsSWC, ucLoop) + (uint16) GETucPAmbValEE(Rte_Inst_CtApHufTpmsSWC)) - 40) > (uint16) 252) {tRdcData[ucLoop].tRecCddRdcData.RDC_DT_1 = cReAkPresOverflow;}
            else                                                                                                                                                {tRdcData[ucLoop].tRecCddRdcData.RDC_DT_1 = (GETucLastWuPressureEE(Rte_Inst_CtApHufTpmsSWC, ucLoop) + GETucPAmbValEE(Rte_Inst_CtApHufTpmsSWC)) - 38;}
         }

         if(GETscLastWuTemperatureEE(Rte_Inst_CtApHufTpmsSWC, ucLoop) == cInvalidREtemperature){tRdcData[ucLoop].tRecCddRdcData.RDC_DT_2 = 20 + 52;}
         else                                                                                  {tRdcData[ucLoop].tRecCddRdcData.RDC_DT_2 = (uint8) GETscLastWuTemperatureEE( Rte_Inst_CtApHufTpmsSWC, ucLoop) + 52;}
...
void PutRecCddRdcData(uint8 ucRe){
   ...
   ushRecEvent[ucRe]++;
   if((uint16) false == PutCddRdcData(ptData)){
   }
}
...
uint8 ucGetNextValidAliveCounter(uint8 ucAliveStartValue){
   ...
   if(ucAliveStartValue < 0x0E){ucAliveCounter = ucAliveStartValue + 1;}
   else                        {ucAliveCounter = 0;}
